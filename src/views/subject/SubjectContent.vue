    ...
    <div class="commonpositiontop scindex">
        <div id="chaptersummary">
            <Card :style="{'margin-left':(collAndshadow?'-320px':0)}">
                <Alert type="warning" v-if="subject.reproduce" show-icon closable>
                ...
                <Timeline>
                    <TimelineItem :color="item.id==$route.params.id?'red':'green'" v-for="item in summary"
                                  :key="item.id" @click.native="changechapter(item.id)"
...
<script>
    ...
    import {getChapterSummary, getSubjectDetail, getChapter} from '@/services/api'
